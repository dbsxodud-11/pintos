...
fat_create_chain (cluster_t clst) {
	...
	// Find Empty Cluster
	cluster_t next_clst;
	for (next_clst=1; next_clst<=fat_fs->fat_length; next_clst++) {
		if (fat_get (next_clst) == 0)
			break;
	}
	...
	if (clst == 0)
		...
		// Extend a chain
		cluster_t past_next_clst = fat_get (clst);
		fat_put (clst, next_clst);
		fat_put (next_clst, past_next_clst);
	}
